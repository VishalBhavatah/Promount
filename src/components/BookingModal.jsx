...
import { fetchCouponCodeFromPromoCode } from '@/lib/zohoInvoicePromo';
...
const BookingModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    ...
    const loadPromoCode = async () => {
      ...
      try {
    //     const promo = await await fetch(
    // `https://promountbackend-914264443.development.catalystserverless.com/server/pro_mount_backend_function/getCouponCode?promoCode=${bookPromoCode}`,
    // {
    //   method: "GET",
    //   headers: {
    ...
    // }
        //     );
        const promo = await fetchCouponCodeFromPromoCode(bookPromoCode);
        if (isMounted) {
